...
function nextPage(){
  if(!slide_loop && slide_current_index + 1 == slides.length)return;
  if(!$('html').is(':animated')){
    var next = (slide_current_index + 1) % slides.length;
    var pos = slides[next].offsetTop + slides[next].offsetHeight/2 - $(window).height()/2;
    if(next == 0) pos = 0;
    if(next + 1 == slides.length) pos = $(document).height() - $(window).height();
    $("html,body").animate({scrollTop:pos});
    slide_current_index = next;
  }
}

function prevPage(){
  if(!slide_loop && slide_current_index == 0)return;
  if(!$('html').is(':animated')){
...
$(function(){
  $("#slides").prepend('<div class="slide" style="opacity:0;"></div>');
  $("#slides").append('<div class="slide" style="opacity:0;"></div>');
  slides = $("#slides > .slide");
  slide_current = $("#slides > .slide")[0];
  slide_current.setAttribute("class", "slide active");
